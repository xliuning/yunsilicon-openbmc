...
# =====================================================================
# diagnostic：stop at initramfs to check MTD sector
# =====================================================================
echo "========================================================"
echo "DEBUG: Entering early initramfs emergency shell..."
echo "Current MTD partitions in /proc/mtd:"
cat /proc/mtd
echo "========================================================"
/bin/sh
# =====================================================================
...
get_mtd_num() {
    local name="$1"
    # get /proc/mtd  "rofs" 或 "rwfs" mapping
    grep "\"${name}\"" /proc/mtd | sed -n 's/mtd\([0-9]*\):.*/\1/p'
}

ro_num=$(get_mtd_num "rofs")
rw_num=$(get_mtd_num "rwfs")

# /proc/mtd not found, use findmtd instead
if [ -z "$ro_num" ]; then
    rofs=$(findmtd rofs)
    ro_num=${rofs#mtd}
fi
...
rofs="mtd${ro_num}"
rwfs="mtd${rw_num}"
rodev="/dev/mtdblock${ro_num}"
rwdev="/dev/mtdblock${rw_num}"

# do not mount if not find, otherwise kernel Panic
if [ -z "$ro_num" ] || [ -z "$rw_num" ]; then
    echo "FATAL ERROR: Could not find 'rofs' or 'rwfs' MTD partitions in /proc/mtd!"
    echo "Current /proc/mtd contents:"
    cat /proc/mtd
    debug_takeover "MTD Partition lookup failed."
fi

# =====================================================================
# 2. erase if mount fail
# =====================================================================
if [ -b "$rwdev" ]; then
    # try mount
    if ! mount "$rwdev" "$rwdir" -t "$rwfst" -o "$rwopts" 2>/dev/null; then
        echo "========================================================"
        echo "WARNING: RWFS ($rwdev) mount failed! Executing flash_eraseall..."
        echo "========================================================"
        flash_eraseall "/dev/$rwfs"
        rwfst=jffs2
        mount "$rwdev" "$rwdir" -t "$rwfst" -o "$rwopts"
    else
        umount "$rwdir" 2>/dev/null
    fi
fi
...
if [ -n "$sel_num" ]; then
    ...
    mkdir -p "$seldir"
    if ! mount "$seldev" "$seldir" -t jffs2 2>/dev/null; then
        echo "WARNING: SEL partition ($seldev) corrupted! Wiping..."
        flash_eraseall "/dev/mtd${sel_num}"
    else
        umount "$seldir" 2>/dev/null
    fi
fi
# =====================================================================
