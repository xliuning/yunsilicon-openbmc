...
get_mtd_num() {
	local name="$1"
	# get /proc/mtd  "rofs" or "rwfs" mapping
	grep "\"${name}\"" /proc/mtd | sed -n 's/mtd\([0-9]*\):.*/\1/p'
}

# =====================================================================
# Pre-check: verify rofs/rwfs MTD partitions exist before proceeding.
# This only reads /proc/mtd; it does not mount or modify anything.
# =====================================================================
ro_num=$(get_mtd_num "rofs")
rw_num=$(get_mtd_num "rwfs")

# /proc/mtd not found or entry missing, fall back to findmtd
if [ -z "$ro_num" ]; then
	rofs=$(findmtd rofs)
	ro_num=${rofs#mtd}
fi
...
then
	msg="Mounting read-write $rwdev filesystem failed.  Please fix and run
